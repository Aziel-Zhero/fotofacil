...
export function AlbumDetailClient({ albumId }: { albumId: string }) {
  ...
  const fetchAlbumData = useCallback(async () => {
    ...
    if (albumResult.error || !albumResult.data) {
      ...
      setAlbum(albumResult.data as AlbumData);
    }
  ...
  const handleUploadFinished = () => {
    // Re-fetch album data to update photo list and album status
    fetchAlbumData();
  }
  ...
  const handleNotifyClient = async () => {
    const result = await notifyClient(albumId);
     if (result?.error) {
      ...
      // Re-fetch data to show the new album status
      fetchAlbumData();
    }
  }
    ...
    <div className="container mx-auto py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
                ...
                <PhotoUploader onUploadFinished={handleUploadFinished} albumId={albumId} />
            </div>
            ...
            <div className='space-y-4'>
                <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold font-headline text-white">Galeria ({photos.length} fotos)</h2>
                     {photos.length > 0 && (
                        <ToggleGroup type="single" value={viewMode} onValueChange={(value) => value && setViewMode(value as ViewMode)} aria-label="Modo de Visualização">
             ...
             <div className="mt-12">
                <Alert>
                    ...
                    <AlertDescription className="flex flex-col sm:flex-row justify-between items-center gap-4">
                       <span>Quando terminar de enviar todas as fotos, notifique seu cliente para que ele possa começar a seleção.</span>
                       <Button onClick={handleNotifyClient} className="w-full sm:w-auto">
