...
-- LIMPEZA COMPLETA E NA ORDEM CORRETA PARA EVITAR ERROS DE DEPENDÊNCIA

-- 1. Remover gatilhos (primeiro o antigo, depois o novo, para garantir)
DROP TRIGGER IF EXISTS on_auth_user_created_photographer ON auth.users;
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;

-- 2. Remover funções (primeiro a antiga, depois a nova)
DROP FUNCTION IF EXISTS public.handle_new_photographer();
DROP FUNCTION IF EXISTS public.handle_new_user();

-- 3. Remover tabelas (CASCADE remove dependências como chaves estrangeiras)
DROP TABLE IF EXISTS public.clients CASCADE;
DROP TABLE IF EXISTS public.photographers CASCADE;

-- 4. Remover tipos customizados
DROP TYPE IF EXISTS public.user_role;


-- RECRIAÇÃO DA ESTRUTURA DO ZERO
...
CREATE POLICY "Photographers can manage their own clients"
  ...
  FOR ALL
  USING (
    (SELECT id FROM public.photographers WHERE user_id = auth.uid()) = photographer_id
  );

-- Função ÚNICA para criar perfil (fotógrafo ou cliente) automaticamente
